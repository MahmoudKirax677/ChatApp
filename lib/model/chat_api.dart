...
extension ChatApi on BuildContext{

  Future getCahtUsers()async{
  ...
  Future getCahtMessage({id,offset,limit})async{
    LoadingDialog().showDialogBox(this);
    Dio dio = Dio();
    dio.options.baseUrl = Url.api_url;
    dio.options.receiveTimeout = const Duration(seconds: 5);
    ...
    dio.options.responseType = ResponseType.json;
    try{
      var response = await dio.request("/user-message?id=$id&offset=$offset&limit=$limit");
      if(response.statusCode == 200){
        LoadingDialog().hideDialog(this);
        return response.data;
      }else {
  ...
  Future storeMessage({chatData})async{
    Dio dio = Dio();
    dio.options.baseUrl = Url.api_url;
    dio.options.receiveTimeout = const Duration(seconds: 5);
    dio.options.connectTimeout = const Duration(seconds: 10);
    ...
    dio.options.responseType = ResponseType.json;
    try{
      var response = await dio.request("/store-message",data: json.encode(chatData));
      if(response.statusCode == 200){
        return response.data;
      }else {
        return false;
